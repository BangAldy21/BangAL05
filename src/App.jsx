...
// --- Firebase (optional; works if you fill the config) ---
// npm i firebase
...
// ---- Firebase config (ISI lalu fitur chat realtime & login aktif) ----
// Rekomendasi: gunakan env variables Vite (VITE_*)
...
function useFirebase() {
  const enabled = !!firebaseConfig.apiKey;
  const app = useMemo(() => (enabled ? initializeApp(firebaseConfig) : null), [enabled]);
  const db = useMemo(() => (app ? getFirestore(app) : null), [app]);
  const auth = useMemo(() => (app ? getAuth(app) : null), [app]);
  return { enabled, db, auth } as const;
}
...
function Card({ children, className = "", onClick }) {
  return (
    <div
      onClick={onClick}
      className={`rounded-2xl p-6 bg-white/5 backdrop-blur border border-white/10 shadow-[0_0_0_1px_rgba(148,163,184,0.12)] ${className}`}
    >
      {children}
    </div>
  );
    ...
    <header className="sticky top-0 z-40 backdrop-blur bg-black/30 border-b border-white/10">
      <nav className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
        ...
        <div className="flex items-center gap-2 md:hidden">
          ...
          <button className="md:hidden text-white" onClick={() => setOpen((v) => !v)}>☰</button>
        </div>
        <div className={`items-center gap-6 text-sm text-slate-300 md:flex ${open ? "flex" : "hidden"}`}>
          <a href="#about" className="hover:text-white">About</a>
          <a href="#tools" className="hover:text-white">Tools</a>
          <a href="#projects" className="hover:text-white">Project</a>
          <a href="#contact" className="hover:text-white">Contact</a>
          <div className="hidden md:block"><ThemeButton dark={dark} setDark={setDark} /></div>
        </div>
      </nav>
    </header>
  );
...
function ThemeButton({ dark, setDark }) {
  return (
    <button
      ...
      className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-3 py-1 text-xs text-slate-200 hover:bg-white/10"
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6 }} className="justify-self-center">
          <div className="relative">
            ...
            <Card className="relative w-72 sm:w-80">
              ...
              <div className="mt-4">
                <h3 className="text-xl font-semibold text-white">{ME.name}</h3>
                <p className="text-sm text-slate-300">{ME.role}</p>
              </div>
      ...
      <div className="mx-auto max-w-6xl px-4 grid gap-8 md:grid-cols-2" ref={ref}>
        ...
        <Card className="min-h-[20rem] grid place-items-center">
          <div className="w-full">
            <div className="mb-3 text-sm text-slate-400">3D Card (.glb)</div>
            {/* Pastikan file public/card.glb ada. Kamu sudah upload card.glb */}
            <model-viewer
              src="/card.glb"
              alt="3D Card"
              auto-rotate
...
======================== */
function ChatRoom() {
  const { enabled, db, auth } = useFirebase();
  const [messages, setMessages] = useState([
    { id: 1, user: "System", text: enabled ? "Menghubungkan ke chat…" : "Mode demo aktif (Firebase belum dikonfigurasi)" },
  ]);
  ...
  useEffect(() => {
    ...
    const qRef = query(collection(db, "messages"), orderBy("createdAt"));
  ...
  const send = async (e) => {
    ...
    if (!enabled || !db) {
      // fallback demo (lokal)
      setMessages((m) => [...m, { id: Date.now(), user: user?.displayName || "Guest", text }]);
      setText("");
      return;
    }
  ...
  return (
    <Card>
      <div className="flex items-center justify-between">
        ...
        {user ? (
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-300">Hi, {user.displayName || "User"}</span>
            <button onClick={logout} className="rounded-full border border-white/15 bg-white/5 px-3 py-1 text-xs text-slate-300 hover:bg-white/10">Logout</button>
          </div>
        ) : (
          <button onClick={login} className="rounded-full border border-white/15 bg-white/5 px-3 py-1 text-xs text-slate-300 hover:bg-white/10">Login with Google</button>
        )}
      </div>
      <div className="mt-3 h-52 overflow-y-auto space-y-2 pr-1">
        {messages.map((m) => (
      ...
      {!enabled && (
        <p className="mt-2 text-xs text-slate-400">Tip: isi VITE_FIREBASE_* di .env agar chat realtime aktif.</p>
      )}
    </Card>
  );
}
